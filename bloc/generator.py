...
def friendship_lookup(oauth, source_screen_name, target_screen_name, msg=""):
    ...
    try:
        response = oauth.get(
            "https://api.twitter.com/1.1/friendships/show.json", params=params
        )
...
def get_timeline_tweets(
    ...
    user_id="",
    max_pages=1,
    following_lookup=True,
    timeline_startdate="",
    timeline_scroll_by_hours=None,
):
    if max_pages < 0:
        return []
    ...
    timeline_startdate = (
        ...
        else datetime.strptime(timeline_startdate, "%Y-%m-%d %H:%M:%S")
    )
    ...
    for i in range(max_pages):
        ...
        if timeline_startdate != "":
            try:
                timeline_next_date = timeline_startdate + timedelta(
                    hours=timeline_scroll_by_hours
                )
                ...
                logger.info(
                    "\ttimeline_startdate: "
                    + str(timeline_startdate)
                    + " ("
                    + str(timeline_scroll_by_hours)
                    + ") -> "
                    + str(next_twt_id)
                )
        ...
        for j in range(len(timeline)):
            timeline[j].setdefault(
                "bloc", {"src_follows_tgt": None, "tgt_follows_src": None}
            )
        ...
        if following_lookup is True:
            ...
            for j in range(timeline_len):
                ...
                if len(relationship) != 0:
                    ...
                    twt["bloc"]["tgt_follows_src"] = relationship["source"][
                        "followed_by"
                    ]
...
def v2_get_timeline_tweets(
    ...
    timeline_scroll_by_hours = (
        0 if timeline_scroll_by_hours is None else timeline_scroll_by_hours
    )
    timeline_startdate = (
        ""
        if timeline_startdate == ""
    ...
    for i in range(max_pages):
        ...
        if timeline_startdate != "":
            logger.info(
                f"\tfrom: {timeline_startdate} (timeline_scroll_by_hours: {timeline_scroll_by_hours})"
            )
        ...
        try:
            end_time = (
                None
                if timeline_startdate == ""
                else timeline_startdate.strftime("%Y-%m-%dT%H:%M:%S") + "Z"
            )
            response = ostwt.get_tweet_timeline(
                user_id,
        ...
        # dedup tweets - end
        logger.info(
            "\t{} tweets, {} errors".format(
                len(tweets), len(response.get("errors", []))
            )
        )
        ...
        if timeline_startdate != "" and timeline_scroll_by_hours != 0:
            timeline_startdate = datetime.strptime(
                response["data"][-1]["created_at"], "%Y-%m-%dT%H:%M:%S.000Z"
            )
            timeline_startdate = timeline_startdate + timedelta(
                hours=timeline_scroll_by_hours
            )
            next_token = None
            logger.info(
                "\ttime pagination, setting next_token to None"
            )  # time pagination and next_token are mutually exclusive
...
def get_duration_prefix(symbols, cur_time, prev_time, blank_mark, minute_mark):
    seq_diff_time = datetime.strptime(
    ...
    dur_glyph = get_delta_glyph(
        symbols, delta_seconds, blank_mark=blank_mark, minute_mark=minute_mark
    )
...
def get_pause(symbols, twt, prev_twt, blank_mark, minute_mark, use_src_ref_time=False):
    ...
    if use_src_ref_time is True:
        if twt["in_reply_to_status_id"] is not None:  # reply
            source_ref_time = find_tweet_timestamp_post_snowflake(
                twt["in_reply_to_status_id"]
            )
            ...
            source_ref_time = datetime.strptime(
                source_ref_time, "%a %b %d %H:%M:%S %z %Y"
            )
            source_ref_time = datetimeFromUtcToLocal(source_ref_time)
            source_ref_time = datetime.strftime(source_ref_time, "%Y-%m-%d %H:%M:%S")
    ...
    if use_src_ref_time is True and source_ref_time != "":
        ...
        delta_seconds, dur_glyph = get_duration_prefix(
            symbols=symbols,
            cur_time=twt["bloc"]["local_time"],
            prev_time=source_ref_time,
        ...
        delta_seconds, dur_glyph = get_duration_prefix(
            symbols=symbols,
            cur_time=twt["bloc"]["local_time"],
            prev_time=prev_twt["bloc"]["local_time"],
            blank_mark=blank_mark,
            minute_mark=minute_mark,
        )
...
def get_bloc_action_seq(
    symbols, twt, delta_seconds, dur_glyph, content_syntactic_seq=""
):
    """
    ...
    if twt.get("in_reply_to_status_id", None) is not None:
        ...
        action_seq = {
            ...
            "note": note,
            "seq_dets": {
                "in_reply_to_screen_name": twt.get("in_reply_to_screen_name", "")
            },
        }
...
def get_bloc_content_sem_sent_seq(
    symbols, twt, txt_key, delta_seconds, dur_glyph, content_syntactic_seq=""
):
    """
    ...
    exclusive_text = ""
    sent_seq = {"delta_seconds": -1, "seq": "", "note": "", "seq_dets": {}}
...
def get_bloc_change_seq(
        ...
        return [
            {
                ...
                "note": symbol_note,
                "details": {
                    "current_statuses_count": cur_u["statuses_count"],
                    "previous_statuses_count": prev_u["statuses_count"],
                },
            }
        ]

    def add_gen_tweet_count_change(
        key, pos_glyph, neg_glyph, pos_label, neg_label, prev_u, cur_u, fold_start_count
    ):
        ...
        return [
            {
                ...
                "note": note,
                "details": {
                    "current_" + key: cur_u[key],
                    "previous_" + key: prev_u[key],
                },
            }
        ]
    ...
    def add_geo_change(glyph, prev_u, cur_u):
        ...
        if "coordinate" in prev_u and "coordinate" in cur_u:
            ...
            if coordinates.count(None) == 2:
                ...
                # both locations are defined
                if (
                    coordinates[0]["coordinates"]["type"] == "Point"
                    and coordinates[1]["coordinates"]["type"] == "Point"
                ):
                    if (
                        coordinates[0]["coordinates"]["coordinates"][0]
                        == coordinates[1]["coordinates"]["coordinates"][0]
                        and coordinates[0]["coordinates"]["coordinates"][1]
                        == coordinates[1]["coordinates"]["coordinates"][1]
                    ):
                        response = []
                    else:
                        response = [
    ...
    """
        ...
        To do:
            test add_geo_change()
    """
    result = {"delta_seconds": -1, "seq": "", "seq_dets": []}
    ...
    seq += add_something_change_v1(
        key="screen_name",
        glyph=symbols["handle_change"]["symbol"],
        prev_u=prev_twt["user"],
        cur_u=twt["user"],
    )
    seq += add_something_change_v1(
        key="name",
        glyph=symbols["name_change"]["symbol"],
        prev_u=prev_twt["user"],
        cur_u=twt["user"],
    )
    seq += add_something_change_v1(
        key="location",
        glyph=symbols["profile_location_change"]["symbol"],
        prev_u=prev_twt["user"],
        cur_u=twt["user"],
    )
    seq += add_geo_change(
        glyph=symbols["geo_location_change"]["symbol"], prev_u=prev_twt, cur_u=twt
    )
    seq += add_profile_appearance_change(
        symbols["profile_appearance_change"]["symbol"],
        symbols["profile_appearance_change"]["description"],
        prev_twt["user"],
        twt["user"],
    )
    seq += add_something_change_v1(
        key="url",
        glyph=symbols["profile_url_change"]["symbol"],
        prev_u=prev_twt["user"],
        cur_u=twt["user"],
    )
    seq += add_something_change_v1(
        key="description",
        glyph=symbols["description_change"]["symbol"],
        prev_u=prev_twt["user"],
        cur_u=twt["user"],
    )
    seq += add_something_change_v2(
        key="source",
        glyph=symbols["source_change"]["symbol"],
        prev_val=prev_twt.get("source", None),
        cur_val=twt.get("source", None),
    )
    seq += add_something_change_v2(
        key="lang",
        glyph=symbols["language_change"]["symbol"],
        prev_val=prev_twt.get("lang", None),
        cur_val=twt.get("lang", None),
    )

    seq += add_delete_tweet_change(
        symbols["delete_tweet"]["symbol"],
        symbols["delete_tweet"]["description"],
        prev_twt["user"],
        twt["user"],
        fold_start_count,
    )
    seq += add_gen_tweet_count_change(
        key="friends_count",
        pos_glyph=symbols["follow_someone"]["symbol"],
        neg_glyph=symbols["unfollow_someone"]["symbol"],
        pos_label=symbols["follow_someone"]["description"],
        neg_label=symbols["unfollow_someone"]["description"],
        prev_u=prev_twt["user"],
        cur_u=twt["user"],
        fold_start_count=fold_start_count,
    )
    seq += add_gen_tweet_count_change(
        key="favourites_count",
        pos_glyph=symbols["like_tweet"]["symbol"],
        neg_glyph=symbols["unlike_tweet"]["symbol"],
        pos_label=symbols["like_tweet"]["description"],
        neg_label=symbols["unlike_tweet"]["description"],
        prev_u=prev_twt["user"],
        cur_u=twt["user"],
        fold_start_count=fold_start_count,
    )
    seq += add_gen_tweet_count_change(
        key="followers_count",
        pos_glyph=symbols["follower_gain"]["symbol"],
        neg_glyph=symbols["follower_loss"]["symbol"],
        pos_label=symbols["follower_gain"]["description"],
        neg_label=symbols["follower_loss"]["description"],
        prev_u=prev_twt["user"],
        cur_u=twt["user"],
        fold_start_count=fold_start_count,
    )
...
def get_bloc_content_syn_seq(
    symbols,
    tweet,
    content_syntactic_add_pause=False,
    txt_key="full_text",
    gen_rt_content=True,
    add_txt_glyph=True,
    delta_seconds=-1,
    ...
    if "entities" in twt:
        ...
        if "user_mentions" in twt["entities"]:
            ...
            if (
                twt["in_reply_to_status_id"] is None
                and len(twt["entities"]["user_mentions"]) != 0
            ):
                # this is NOT a reply
        ...
        if txt_key in twt:
            ...
            if "extended_entities" in twt:
                exclusive_text = get_twt_text_exclusively(
                    exclusive_text, twt["extended_entities"]
                )
    ...
    result = {"seq": "".join([l["label"] for l in seq]), "seq_dets": seq}

    result["delta_seconds"] = delta_seconds

    if result["seq"] != "":
        result["seq"] = (
            dur_glyph + "(" + result["seq"] + ")"
            if content_syntactic_add_pause is True
            else "(" + result["seq"] + ")"
        )
...
def get_bloc_content_sem_ent_seq(
    symbols,
    tweet,
    content_semantic_add_pause=False,
    gen_rt_content=True,
    delta_seconds=-1,
    dur_glyph="",
):
    ...
    if result["seq"] != "":
        result["seq"] = (
            dur_glyph + "(" + result["seq"] + ")"
            if content_semantic_add_pause is True
            else "(" + result["seq"] + ")"
        )
...
def bloc_segmenter(
    ...
    days_segment_count=-1,
):
    bloc_info["local_time"] = datetime.strftime(local_time, "%Y-%m-%d %H:%M:%S")
    bloc_info["week_number"] = (
        format(local_time.year, "04d")
        + "."
        + format(local_time.isocalendar()[1], "03d")
    )
    ...
    if days_segment_count > 0:
        ...
        bloc_info["day_of_year_bin"] = (
            bloc_info["day_of_year"] - 1
        ) // days_segment_count

        bloc_info["day_of_year"] = (
            format(local_time.year, "04d")
            + "."
            + format(bloc_info["day_of_year"], "03d")
        )
        bloc_info["day_of_year_bin"] = (
            format(local_time.year, "04d")
            + "."
            + format(bloc_info["day_of_year_bin"], "03d")
        )

    bloc_info["segmentation_type"] = segmentation_type


def add_bloc_sequences(
    tweets,
    blank_mark=60,
    minute_mark=5,
    gen_rt_content=True,
    ...
    **kwargs,
    ...
    def add_bloc_segments(twt, segment_id, bloc_segments):
        ...
        for dim, dim_dct in twt["bloc"]["bloc_sequences"].items():
            ...
            if dim in bloc_segments["segments"][segment_id]:
                bloc_segments["segments"][segment_id][dim] += twt["bloc"][
                    "bloc_sequences"
                ][dim]["seq"]
            else:
                bloc_segments["segments"][segment_id][dim] = twt["bloc"][
    ...
    def fmt_action_content_syntactic_bloc(symbols, bloc_segments, alph_key):
        ...
        time_glyphs = [
            symb[1]["symbol"] for symb in symbols.items() if symb[0] != "blank_mark"
        ]
    ...
    kwargs.setdefault(
        "time_reference", "previous_tweet"
    )  # previous_tweet or reference_tweet
    ...
    bloc_alphabets = kwargs.get(
        "bloc_alphabets", ["action", "content_syntactic", "content_semantic_entity"]
    )  # additional valid bloc_alphabets: change, content_syntactic_with_pauses, action_content_syntactic

    segmentation_type = (
        segmentation_type
        if segmentation_type in ["week_number", "day_of_year_bin", "yyyy-mm-dd"]
        else "week_number"
    )
    bloc_segments = {
        "segments": {},
    ...
    for i in range(twt_len):
        ...
        bloc_segmenter(
            twt["bloc"],
            created_at,
            datetimeFromUtcToLocal(created_at),
            segmentation_type=segmentation_type,
            days_segment_count=days_segment_count,
        )
    ...
    for i in range(twt_len):
        ...
        delta_seconds, dur_glyph = get_pause(
            symbols=all_bloc_symbols["bloc_alphabets"]["time"],
            twt=twt,
            prev_twt=prev_twt,
            blank_mark=blank_mark,
            minute_mark=minute_mark,
            use_src_ref_time=use_src_ref_time,
        )
        twt["bloc"]["bloc_sequences"] = {}

        if "action" in bloc_alphabets:
            twt["bloc"]["bloc_sequences"]["action"] = get_bloc_action_seq(
                all_bloc_symbols["bloc_alphabets"]["action"],
                twt,
                delta_seconds,
                dur_glyph,
            )
        ...
        if "content_syntactic" in bloc_alphabets:
            twt["bloc"]["bloc_sequences"][
                "content_syntactic"
            ] = get_bloc_content_syn_seq(
                all_bloc_symbols["bloc_alphabets"]["content_syntactic"],
                twt,
                content_syntactic_add_pause=False,
                txt_key=twt_text_ky,
                gen_rt_content=gen_rt_content,
                add_txt_glyph=add_txt_glyph,
                delta_seconds=delta_seconds,
                dur_glyph=dur_glyph,
            )
        ...
        if "content_syntactic_with_pauses" in bloc_alphabets:
            twt["bloc"]["bloc_sequences"][
                "content_syntactic_with_pauses"
            ] = get_bloc_content_syn_seq(
                all_bloc_symbols["bloc_alphabets"]["content_syntactic"],
                twt,
                content_syntactic_add_pause=True,
                txt_key=twt_text_ky,
                gen_rt_content=gen_rt_content,
                add_txt_glyph=add_txt_glyph,
                delta_seconds=delta_seconds,
        ...
        if "content_semantic_entity" in bloc_alphabets:
            ...
            ] = get_bloc_content_sem_ent_seq(
                all_bloc_symbols["bloc_alphabets"]["content_semantic_entities"],
                twt,
                content_semantic_add_pause=False,
                gen_rt_content=gen_rt_content,
                delta_seconds=delta_seconds,
        ...
        if "content_semantic_sentiment" in bloc_alphabets:
            ...
            ] = get_bloc_content_sem_sent_seq(
                all_bloc_symbols["bloc_alphabets"]["content_semantic_sentiment"],
                twt,
                txt_key=twt_text_ky,
                delta_seconds=delta_seconds,
                dur_glyph=dur_glyph,
                content_syntactic_seq="",
            )

        if "change" in bloc_alphabets:
            twt["bloc"]["bloc_sequences"]["change"] = get_bloc_change_seq(
                all_bloc_symbols["bloc_alphabets"]["change"],
                twt,
                prev_twt,
                delta_seconds=delta_seconds,
                dur_glyph=dur_glyph,
                fold_start_count=kwargs["fold_start_count"],
                include_time=kwargs["change_add_pause"],
            )

        if "action_content_syntactic" in bloc_alphabets:
            tmp_cnt = get_bloc_content_syn_seq(
                all_bloc_symbols["bloc_alphabets"]["content_syntactic"],
                twt,
                content_syntactic_add_pause=False,
                txt_key=twt_text_ky,
            ...
            twt["bloc"]["bloc_sequences"][
                ...
                delta_seconds,
                dur_glyph,
                content_syntactic_seq=tmp_cnt["seq"],
            )
    ...
    if "action_content_syntactic" in bloc_alphabets:
        fmt_action_content_syntactic_bloc(
            all_bloc_symbols["bloc_alphabets"]["time"],
...
def post_proc_bloc_sequences(
    report, seconds_mark, minute_mark, ansi_code, segmentation_type
):
...
def get_timeline_request_dets(report):
    ...
    try:
        screen_name = (
            ...
            else tweets[0]["user"]["screen_name"]
        )
        st_time = datetime.strptime(
            tweets[0]["bloc"]["local_time"], "%Y-%m-%d %H:%M:%S"
        )
        en_time = datetime.strptime(
            tweets[-1]["bloc"]["local_time"], "%Y-%m-%d %H:%M:%S"
        )
...
def get_timeline_tweets_cache_filename(
    cache_path,
    screen_name,
    max_pages,
    following_lookup,
...
def get_user_bloc(
    ...
    **kwargs,
    ...
    if screen_name == "":
        cache_filename = get_timeline_tweets_cache_filename(
            kwargs["cache_path"],
            user_id,
            max_pages=max_pages,
    ...
    if kwargs["cache_path"] != "" and kwargs["cache_read"] is True:
        logger.info(
            "\nget_user_bloc() attempting to read timeline tweets from: "
            + cache_filename
        )
    ...
    post_proc_bloc_sequences(
        payload,
        kwargs["blank_mark"],
        kwargs["minute_mark"],
        kwargs["ansi_code"],
        kwargs.get("segmentation_type", None),
    )
...
def is_symbols_good(all_bloc_symbols):
    ...
    if "bloc_alphabets" in all_bloc_symbols:
        for alph in [
            ...
            "content_syntactic",
            "change",
            "content_semantic_entities",
            "content_semantic_sentiment",
        ]:
            if alph not in all_bloc_symbols["bloc_alphabets"]:
                logger.error(
                    "\nis_symbols_good(): {} alphabet not in {}, returning".format(
                        alph, all_bloc_symbols["bloc_alphabets"].keys()
                    )
                )
                return False
            ...
            if len(all_bloc_symbols["bloc_alphabets"][alph]) == 0:
                logger.error(
                    "\nis_symbols_good(): len(all_bloc_symbols['bloc_alphabets']['{}']) == 0, returning".format(
                        alph
                    )
                )
                return False
...
def gen_bloc_for_users(
    ...
    max_pages=1,
    following_lookup=False,
    timeline_startdate=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    timeline_scroll_by_hours=None,
    ngram=1,
    **kwargs,
):
    ...
    bloc_symbols_file = (
        "{}/symbols.json".format(os.path.dirname(os.path.abspath(__file__)))
        if bloc_symbols_file is None
        else bloc_symbols_file
    )
    ...
    if bearer_token == "":
        ...
        oauth_or_ostwt = osometweet.OsomeTweet(
            osometweet.OAuth2(bearer_token=bearer_token, manage_rate_limits=False)
        )
        if kwargs["no_screen_name"] == False:
            ...
            screen_names_or_ids = twitter_v2_user_lookup_ids(
                oauth_or_ostwt, screen_names_or_ids
            )
