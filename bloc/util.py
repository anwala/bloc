...
def gen_post_snowflake_twitter_id(tweet_datetime):
    ...
    tweet_datetime = (
        tweet_datetime.timestamp() * 1000
    ) - offset  # convert seconds to nanoseconds by * 1000
...
def get_screen_name_frm_status_uri(tweet_uri):
    ...
    if (
        tweet_uri.startswith("https://twitter.com/") is False
        or tweet_uri.find("/status/") == -1
    ):
        return ""
...
def dumpJsonToFile(outfilename, dictToWrite, indentFlag=True, extraParams=None):
    ...
    try:
        ...
        if indentFlag:
            json.dump(
                dictToWrite, outfile, ensure_ascii=False, indent=4
            )  # by default, ensure_ascii=True, and this will cause  all non-ASCII characters in the output are escaped with \uXXXX sequences, and the result is a str instance consisting of ASCII characters only. Since in python 3 all strings are unicode by default, forcing ascii is unecessary
        else:
...
def color_bloc_action_str(
    bloc_action_str, split_pattern="([^□⚀⚁⚂⚃⚄⚅.|()*]+)", ansi_code="91m"
):
    if ansi_code.strip() == "":
...
def map_tf_mat_to_doc_ids(payload, pos_id_mapping):
    ...
    for pos, doc_dct in pos_id_mapping.items():
        if pos < len(payload["tf_matrix"]):
            tf_matrix.append(
                {"id": doc_dct["id"], "tf_vector": payload["tf_matrix"][pos]}
            )
        ...
        if opt in payload:
            ...
            for pos, doc_dct in pos_id_mapping.items():
                if pos < len(payload[opt]):
                    opt_vect.append(
                        {"id": doc_dct["id"], "tf_vector": payload[opt][pos]}
                    )
...
def gen_folded_vocab(tf_matrix, bloc_variant):
    ...
    for i in range(len(tf_matrix["vocab"])):
        ...
        new_v = fold_word(
            v,
            bloc_variant["fold_start_count"],
            count_applies_to_all_char=bloc_variant["count_applies_to_all_char"],
            fold_exclude_chars=bloc_variant.get("fold_exclude_chars", ""),
        )
...
def get_default_symbols():
    bloc_symbols_file = "{}/symbols.json".format(
...
def gen_bloc_variant_tf_mat(tf_matrix, bloc_variant):
    if (
        "tf_matrix" not in tf_matrix
        or "vocab" not in tf_matrix
        or "type" not in bloc_variant
    ):
        return {}
    ...
    logger.debug(
        "\tcount_applies_to_all_char: {}".format(
            bloc_variant["count_applies_to_all_char"]
        )
    )
    ...
    for i in range(1, len(tf_matrix["tf_matrix"])):
        tf_mat = sp.vstack(
            (tf_mat, tf_matrix["tf_matrix"][i]["tf_vector"]), format="csr"
        )
    ...
    for v, col_indices in new_vocab.items():
        logger.debug(
            "\tfeature_indx: {} {} col_indices: {}".format(feature_indx, v, col_indices)
        )
        feature_indx += 1
    ...
    return {"tf_matrix": new_tf_mat, "vocab": new_vocab_lst}


def update_bloc_model(
    old_bloc_model, new_tf_mat, tf_matrix_norm, tf_idf_norm, **kwargs
):
    if len(new_tf_mat) == 0:
    ...
    for m in ["per_doc", "all_docs"]:
        ...
        for i in range(len(new_tf_mat["top_ngrams"][m])):
            old_bloc_model["top_ngrams"][m][i]["ngrams"] = new_tf_mat["top_ngrams"][m][
                i
            ]
...
def get_bloc_variant_tf_matrix(
    doc_lst,
    ngram,
    tf_mat=None,
    vocab=None,
    ...
    if bloc_variant is None:
        ...
        for m in ["tf_matrix_normalized", "tf_idf_matrix"]:
            ...
            for i in range(len(tf_matrix["tf_matrix"])):
                doc_dct = [
                    (v[0], v[1])
                    for v in tf_matrix["tf_matrix"][i].items()
                    if v[0] != "tf_vector"
                ]
        ...
        tf_matrix = update_bloc_model(
            tf_matrix,
            bloc_variant_tf_matrix,
            tf_matrix_norm=kwargs.pop("tf_matrix_norm", ""),
            tf_idf_norm=kwargs.pop("tf_idf_norm", "l2"),
            **kwargs,
        )
...
def conv_tf_matrix_to_json_compliant(tf_mat):
    ...
    for opt in ["tf_matrix", "tf_matrix_normalized", "tf_idf_matrix"]:
        ...
        for i in range(len(tf_mat[opt])):
            tf_mat[opt][i]["tf_vector"] = [
                float(a) for a in tf_mat[opt][i]["tf_vector"].toarray()[0]
            ]
...
def get_tf_matrix(
    ...
    tf_mat=None,
    vocab=None,
    token_pattern=r"(?u)\b[a-zA-Z\'\’-]+[a-zA-Z]+\b|\d+[.,]?\d*",
    **kwargs,
):
    ...
    if isinstance(doc_lst, list):
        ...
        if isinstance(doc_lst[0], dict):
            doc_lst, pos_id_mapping = get_doc_lst_pos_maps(
                doc_lst, rm_text=kwargs["rm_doc_text"]
            )
            if len(doc_lst) == 0:
                return {}
    ...
    try:
        ...
        if kwargs["tf_matrix_norm"] != "":
            payload["tf_matrix_normalized"] = normalize(
                payload["tf_matrix"], norm=kwargs["tf_matrix_norm"], axis=1
            )
    ...
    if kwargs["set_top_ngrams"] is True:
        calc_top_ngrams(
            payload, top_ngrams_add_all_docs=kwargs["top_ngrams_add_all_docs"]
        )
...
def calc_top_ngrams(payload, top_ngrams_add_all_docs=False):
    ...
    for i in range(corpus_size):
        ...
        single_doc_tf = [
            {"term": v, "term_freq": int(row[j]), "term_rate": row[j] / total_tf}
            for (j, v) in enumerate(payload["vocab"])
            if row[j] != 0
        ]
        single_doc_tf = sorted(
            single_doc_tf, key=lambda i: i["term_freq"], reverse=True
        )
    ...
    if top_ngrams_add_all_docs is True:
        ...
        payload["top_ngrams"]["all_docs"] = sorted(
            all_docs_tf.items(), key=lambda x: x[1]["df"], reverse=True
        )
        payload["top_ngrams"]["all_docs"] = [
            {
                "term": t[0],
                "term_freq": t[1]["tf"],
                "term_rate": t[1]["tf"] / all_docs_total_tf,
                "doc_freq": t[1]["df"],
                "doc_rate": t[1]["df"] / corpus_size,
            }
            for t in payload["top_ngrams"]["all_docs"]
        ]
...
def segment_paren_word(word, sort_paren=True):
    ...
    for w in word:
        if w == "(":
            ...
            paren_words[-1] = (
                "(" + "".join(sorted(paren_words[-1])) + ")"
                if sort_paren is True
                else "(" + paren_words[-1] + ")"
            )
            paren_words.append("")
            continue
...
def fold_word(
    ...
    if isinstance(word, str):
        word = gen_bloc_folded_letters(
            word, fold_start_count=fold_start_count, sort_paren=sort_paren
        )
...
def twitter_v2_user_lookup_ids(osome_twt_obj, screen_names):
    ...
    for i in range(0, max_users_for_req, 100):
        logger.info(
            f"\tscreen_name lookup {i+1} of {max_users_for_req//100}, total users: {max_users_for_req}"
        )
        ...
        try:
            response = osome_twt_obj.user_lookup_usernames(
                screen_names[i : i + 100], fields=osometweet.UserFields(everything=True)
            )
        except:
            genericErrorInfo()
            continue
