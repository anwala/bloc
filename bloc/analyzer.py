...
def generate_bloc_expr_template(all_screenames_bloc):
    ...
    for src_segment_number in range(max_segment_number, min_segment_number, -1):
        ...
        for tgt_segment_number in range(
            src_segment_number - 1, min_segment_number - 1, -1
        ):
            experiment_buckets[src_segment_number].append(
                list(range(src_segment_number - 1, tgt_segment_number - 1, -1))
            )
    ...
    for expr_run in src_target_experiment_buckets:
        all_experiments += extract_bloc_from_segments(
            expr_run["testing_segment"], expr_run["training_segments"]
        )
    ...
    for test_seg, train_seg in flat_experiment.items():
        semantic_exper.append(
            {"testing_segment": test_seg, "training_segments": train_seg}
        )
...
def obsolete_get_user_bloc_for_dim(bloc_segments, segment_numbers, seg_dimensions):
    ...
    for seg_num in segment_numbers:
        ...
        for seg_dim in seg_dimensions.split(", "):
            ...
            seg_dimensions_docs = (
                seg_dimensions_docs + " * " + bloc_segments[seg_num][seg_dim]
            )
...
def gen_expr_doc_lst_for_single_user(user, all_experiments, expr_bloc_dims):
    ...
    for expr_run in all_experiments:
        ...
        test_doc_lst = get_user_bloc_for_dim(
            user["bloc_segments"]["segments"], [test_seg], expr_bloc_dims
        )
        ...
        logger.debug(
            "experiment: "
            + str(expr_run["testing_segment"])
            + " vs "
            + str(expr_run["training_segments"])
        )
        expr_run["training_segments"] = rm_absent_segment(
            expr_run["training_segments"], user["bloc_segments"]["segments"]
        )
        logger.debug(
            "experiment: "
            + str(expr_run["testing_segment"])
        ...
        for train in expr_run["training_segments"]:
            train_doc_lst = get_user_bloc_for_dim(
                user["bloc_segments"]["segments"], train, expr_bloc_dims
            )
...
def gen_expr_doc_lst_for_users(all_screenames_bloc, all_experiments, expr_bloc_dims):
    for i in range(len(all_screenames_bloc)):
        ...
        gen_expr_doc_lst_for_single_user(
            user, user["bloc_experiment_docs"], expr_bloc_dims
        )

        # rm empty experiment docs
        user["bloc_experiment_docs"] = [
            doc for doc in user["bloc_experiment_docs"] if len(doc) != 0
        ]


def gen_ngram_tf_matrix(
    all_screenames_bloc,
    ngram,
    token_pattern,
    matrix_key="tf_matrix_normalized",
    tf_matrix_norm="l1",
    tf_idf_norm="l2",
):
    logger.debug("\ngen_ngram_tf_matrix()")
    ...
    for i in range(len(all_screenames_bloc)):
        ...
        for j in range(len(user["bloc_experiment_docs"])):
            ...
            tf_matrix.append(
                {
                    ...
                    "text": combine_text_across_segs_bloc_dims(
                        expr_doc["testing_doc_lst"]["text"]
                    ),
            ...
            for k in range(len(expr_doc["training_doc_lst"])):
                tf_matrix.append(
                    {
                        ...
                        "text": combine_text_across_segs_bloc_dims(
                            expr_doc["training_doc_lst"][k]["text"]
                        ),
                ...
                logger.debug(
                    "\t\ttraining doc: " + str(expr_doc["training_doc_lst"][k])
                )
...
def get_sample_top_ngram(top_per_doc_ngram):
    if len(top_per_doc_ngram) == 0:
        ...
        top_ngrams = (
            "("
            + top_per_doc_ngram[0]["term"]
            + ", "
            + "{0:0.2f}".format(top_per_doc_ngram[0]["term_rate"])
            + ")"
        )
...
def get_markov_chains_for_bloc_dims(segment_seqs):
    ...
    for dim, bloc_seq in mkv_inputs.items():
        mkv_chains[dim] = {
            "model": BLOCMarkovChain(training_sequence_lst=bloc_seq),
            "training_seq": "".join(bloc_seq),
        }
...
def get_src_tgt_mkv_dist(mc_src, mc_tgt):
    ...
    for dim in mc_src:
        if dim in mc_tgt:
            good_prob_flag = True
            prob_src_tgt = BLOCMarkovChain.prob_of_sequence(
                mc_src[dim]["model"].training_model,
                mc_tgt[dim]["training_seq"],
                log_prob=True,
            )
            prob_src_tgt = prob_src_tgt / len(mc_tgt[dim]["training_seq"])

            prob_tgt_src = BLOCMarkovChain.prob_of_sequence(
                mc_tgt[dim]["model"].training_model,
                mc_src[dim]["training_seq"],
                log_prob=True,
            )
...
def pairwise_pred_prob_multi_markvov(tf_matrix):
    ...
    for i in range(X):
        ...
        for j in range(X):
            ...
            total_src_tgt_prob_all_models, good_prob_flag = get_src_tgt_mkv_dist(
                mc_src, mc_tgt
            )
...
def pairwise_pred_prob(tf_matrix):
    ...
    print(
        "\tpairwise_pred_prob(): revise function since BLOCMarkovChain has changed, returning"
    )
    for i in range(X):
        pred_prob_matrix.append([])
        for j in range(X):
    ...
    for i in range(X):
        ...
        for j in range(X):
            ...
            prob_src_tgt = BLOCMarkovChain.s_prob_of_sequence(
                mc_src.training_model, tgt_seq, log_prob=True
            )
            prob_src_tgt = prob_src_tgt / len(tgt_seq)

            prob_tgt_src = BLOCMarkovChain.s_prob_of_sequence(
                mc_tgt.training_model, src_seq, log_prob=True
            )
...
def analyze_ngram_vectors(
    tf_matrix, matrix_key="tf_matrix_normalized", analyze_training_segment_length=1
):
    if matrix_key not in tf_matrix:
        return {
            "experiment_parameters": {"matrix_key": matrix_key},
            "prediction_results": [],
        }
    ...
    for i in range(distance_matrix.shape[0]):
        a = (
            tf_matrix[matrix_key][i]["screen_name"]
            + ", "
            + str(tf_matrix[matrix_key][i]["segments"])
        )
        ...
        logger.debug(
            ...
            + a
            + " test "
            + get_sample_top_ngram(tf_matrix["top_ngrams"]["per_doc"][i]["ngrams"])
        )
        ...
        for j in range(distance_matrix.shape[1]):
            ...
            training_ky = (
                train_tf_mat["screen_name"] + ", " + str(train_tf_mat["segments"])
            )
            ...
            if train_tf_mat["id"] == tf_matrix["top_ngrams"]["per_doc"][j]["id"]:
                top_per_doc_ngram = get_sample_top_ngram(
                    tf_matrix["top_ngrams"]["per_doc"][j]["ngrams"]
                )

            if tf_matrix[matrix_key][i]["screen_name"] != train_tf_mat["screen_name"]:
                pred_payload["test"]["train_non_test_screen_name_count"].add(
                    train_tf_mat["screen_name"]
                )
        ...
        markov_training_samples = sorted(
            training_samples, key=lambda i: i["pred_prob"], reverse=True
        )

        pred_payload["test"]["train_non_test_screen_name_count"] = list(
            pred_payload["test"]["train_non_test_screen_name_count"]
        )
        bloc_prediction_results.append(
            {"test": pred_payload["test"], "train": bloc_training_samples}
        )

        markov_prediction_results.append(
            {"test": pred_payload["test"], "train": markov_training_samples}
        )
        ...
        for j in range(len(bloc_training_samples)):
            ...
            logger.debug(
                ...
                + str(train["segments"])
                + " "
                + train["top_per_doc_ngram"]
            )

        logger.debug("")
        logger.debug(
            "i: " + str(i) + " " + a + " test " + tf_matrix[matrix_key][i]["text"]
        )
...
def report_prediction_results(pred_results, parameters):
    ...
    for test_train in pred_results:
        ...
        if (
            len(test_train["test"]["train_non_test_screen_name_count"]) == 0
            or len(test_train["train"]) == 0
        ):
            continue
        ...
        if (
            test_train["test"]["screen_name"]
            == test_train["train"][guess]["screen_name"]
        ):
    ...
    return {"precision": precision, "random_precision": random_precision}
...
def update_user_test_train_tf_vects(tf_matrix, all_screenames_bloc):
    ...
    for mat in tf_matrix:
        ...
        if "training_doc_indx" in mat:
            ...
            user["bloc_experiment_docs"][j]["training_doc_lst"][k]["tf_vector"] = mat[
                "tf_vector"
            ]

        else:
            user["bloc_experiment_docs"][j]["testing_doc_lst"]["tf_vector"] = mat[
                "tf_vector"
            ]


def analyze_bloc_for_users(
    all_screenames_bloc,
    ngram=1,
    analyze_bloc_dimensions="action",
    analyze_matrix_key="tf_matrix_normalized",
    analyze_training_segment_length=1,
    max_pages=-1,
    timeline_startdate="",
    ...
    **kwargs
    ...
    gen_expr_doc_lst_for_users(
        all_screenames_bloc, deepcopy(all_experiments), analyze_bloc_dimensions
    )
    tf_matrix = gen_ngram_tf_matrix(
        all_screenames_bloc,
        ngram,
        token_pattern=kwargs["token_pattern"],
        matrix_key=analyze_matrix_key,
        tf_matrix_norm=kwargs["analyze_tf_matrix_norm"],
        tf_idf_norm=kwargs["analyze_tf_idf_matrix_norm"],
    )

    if kwargs["bloc_variant"] is not None:
        bloc_variant_tf_matrix = gen_bloc_variant_tf_mat(
            tf_matrix, kwargs["bloc_variant"]
        )
        new_tf_matrix = merge_bloc_matrices(
            tf_matrix, bloc_variant_tf_matrix, kwargs["bloc_variant"]
        )

        # dumpJsonToFile('tf_matrix.json', tf_matrix)

        tf_matrix = update_bloc_model(
            tf_matrix,
            new_tf_matrix,
            tf_matrix_norm=kwargs["analyze_tf_matrix_norm"],
            tf_idf_norm=kwargs["analyze_tf_idf_matrix_norm"],
        ...
        if analyze_matrix_key in tf_matrix:
            update_user_test_train_tf_vects(
                tf_matrix[analyze_matrix_key], all_screenames_bloc
            )

        # dumpJsonToFile('new_tf_matrix.json', tf_matrix)

    pred_res = analyze_ngram_vectors(
        tf_matrix,
        matrix_key=analyze_matrix_key,
        analyze_training_segment_length=analyze_training_segment_length,
    )

    pred_res["experiment_parameters"]["ngram"] = ngram
    pred_res["experiment_parameters"][
        "analyze_training_segment_length"
    ] = analyze_training_segment_length
    pred_res["experiment_parameters"][
        "bloc_experiment_dimensions"
    ] = analyze_bloc_dimensions
    ...
    if len(pred_res["prediction_results"]) != 0:
        pred_report_bloc = report_prediction_results(
            pred_res["prediction_results"]["bloc"], pred_res["experiment_parameters"]
        )
        pred_report_markov = report_prediction_results(
            pred_res["prediction_results"]["markov"], pred_res["experiment_parameters"]
        )
