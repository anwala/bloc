...
def is_invalidate_matrix(matrix_key, tf_mat):
    if matrix_key not in tf_mat or "vocab" not in tf_mat:
        logger.warning(
            f"\t{matrix_key} not in tf_mat, returning, see: " + str(tf_mat.keys())
        )
        return True
...
def get_one_v_rest_centroid(dataset, vector_key, user):
    ...
    return {"centroid": centroid / count, "count": count}
...
def run_pca(matrix, features, n_components=10, z_score_normalize=True):
    ...
    if n_components > n_features:
        warn(
            f"run_pca(): setting n_components = 2 since n_components ({n_components}) > n_features ({n_features})"
        )
        n_components = 2
    ...
    for i in range(len(features)):
        ...
        feature_importance.append({"feature": features[i], "score": score, "indx": i})

    feature_importance = sorted(
        feature_importance, key=lambda x: x["score"], reverse=True
    )
...
def get_bloc_pred_centroids(tf_mat, vocab, n_components=0, **kwargs):
    ...
    if n_components > 0:
        try:
            pca_payload = run_pca(
                ...
                z_score_normalize=kwargs["pca_z_score_normalize"],
            )
        except:
            genericErrorInfo()
        ...
        n_components = len(
            pca_payload["pca_points"][0]["pca_vect"]
        )  # it's possible for n_components to change after run_pca
    ...
    for i in range(len(tf_mat)):
        ...
        centroids.setdefault(
            classname,
            {"centroid": np.zeros(centroid_vect_dim), "count": 0, "class": classname},
        )
    ...
    for classname in centroids:
        centroids[classname]["centroid"] = (
            centroids[classname]["centroid"] / centroids[classname]["count"]
        )

    return {"pca_payload": pca_payload, "centroids": centroids}


def predict_user_class_centroid(
    doc_lst, tf_mat, centroids, pca_payload, one_v_rest=True
):
    ...
    for i in range(len(doc_lst)):
        ...
        if len(pca_payload) == 0:
            ...
            loc_centroid[classname] = (
                loc_centroid[classname]
                if one_v_rest is False
                else get_one_v_rest_centroid(tf_mat, "tf_vector", user)
            )
            pred = predict_user_type_centroid_helper(
                tf_mat[i]["tf_vector"], loc_centroid
            )
        else:
            loc_centroid[classname] = (
                loc_centroid[classname]
                if one_v_rest is False
                else get_one_v_rest_centroid(
                    pca_payload["pca_points"], "pca_vect", user
                )
            )
            pred = predict_user_type_centroid_helper(
                pca_payload["pca_points"][i]["pca_vect"], loc_centroid
            )
    ...
    if len(y_true) != 0 and len(y_pred) != 0:
        class_rep = classification_report(
            y_true, y_pred, output_dict=True, target_names=all_classes
        )
...
def predict_user_class_tfidf(training_doc_lst, test_doc_lst, model, **kwargs):
    if len(training_doc_lst) == 0 or len(test_doc_lst) == 0:
        logger.warning(
            "len(training_doc_lst) == 0 or len(test_doc_lst) == 0, returning"
        )
        return {}
    ...
    if len(all_train_classes - all_test_classes) > 0:
        ...
        logger.warning(
            " all_test_classes has class(es) absent from all_train_classes, returning"
        )
        return {}
    ...
    if is_invalidate_matrix(matrix_key, train_test_tf_mat):
        logger.warning(
            " is_invalidate_matrix( matrix_key, train_test_tf_mat ), returning"
        )
        return {}
    ...
    for i in range(len(test_doc_lst)):
        ...
        classname = u["class"]
        test_tf_mat_indx = (
            len(training_doc_lst) + i
        )  # because training_doc_lst + test_doc_lst
        ...
        pred = predict_user_type_centroid_helper(
            train_test_tf_mat[matrix_key][test_tf_mat_indx]["tf_vector"],
            train_per_src_doc,
            dist_metric=kwargs.get("dist_metric", "cosine"),
        )
    ...
    if len(y_true) != 0 and len(y_pred) != 0:
        class_rep = classification_report(
            y_true, y_pred, output_dict=True, target_names=all_train_classes
        )
...
def predict_user_class_bloc_centroid(
    bloc_model, n_components=0, training_doc_lst=[], test_doc_lst=[], **kwargs
):
    """
    ...
    bloc_model_params = {
        "bigram": {"ngram": 2, "token_pattern": "[^ |()*]", "bloc_variant": None},
        "word": {
            "ngram": 1,
            "token_pattern": "[^□⚀⚁⚂⚃⚄⚅ |()*]+|[□⚀⚁⚂⚃⚄⚅]",
            "bloc_variant": {
    ...
    if len(training_doc_lst) > 0 and len(test_doc_lst) > 0:
        training_tf_mat = get_bloc_tf_matrix(
            training_doc_lst, bloc_model_params[bloc_model]
        )
        ...
        training_cent_pca = get_bloc_pred_centroids(
            training_tf_mat[matrix_key],
            training_tf_mat["vocab"],
            n_components=n_components,
            **kwargs,
        )
        test_cent_pca = get_bloc_pred_centroids(
            test_tf_mat[matrix_key],
            test_tf_mat["vocab"],
            n_components=n_components,
            **kwargs,
        )
        if len(training_cent_pca) == 0 or len(test_cent_pca) == 0:
            return {}
        class_rep = predict_user_class_centroid(
            test_doc_lst,
            test_tf_mat[matrix_key],
            training_cent_pca["centroids"],
            test_cent_pca["pca_payload"],
            one_v_rest=False,
        )
        ...
        cent_pca = get_bloc_pred_centroids(
            tf_mat[matrix_key], tf_mat["vocab"], n_components=n_components, **kwargs
        )
        class_rep = predict_user_class_centroid(
            training_doc_lst,
            tf_mat[matrix_key],
            cent_pca["centroids"],
            cent_pca["pca_payload"],
            one_v_rest=True,
        )
...
def predict_user_class_bloc_tfidf(
    bloc_model, training_doc_lst=[], test_doc_lst=[], **kwargs
):
    if bloc_model not in ["bigram", "word"]:
        logger.warning(f"{bloc_model} not bigram or word, returning")
        return {}
    ...
    bloc_model_params = kwargs.get(
        "bloc_model_params", default_model_params[bloc_model]
    )
    ...
    class_rep = predict_user_class_tfidf(
        training_doc_lst, test_doc_lst, bloc_model_params, **kwargs
    )
