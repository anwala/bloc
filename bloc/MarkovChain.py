...
class BLOCMarkovChain:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if training_sequence_lst is not None:
            ...
            training_model = BLOCMarkovChain.train_markov_model(
                training_sequence_lst, model_output_filename, model_label
            )
        ...
        BLOCMarkovChain.validate_prob_params(
            self.states, self.transition_matrix, self.init_prob_dist
        )
    ...
    def validate_prob_params(states, transition_matrix, init_prob_dist):
        ...
        if transition_matrix is None:
            ...
            for i in range(transition_matrix.shape[0]):
                ...
                if not np.isclose(sm, 1.0):
                    warnings.warn(
                        "Transition probabilities don't sum to 1. Row {}, Sum = {}".format(
                            i, sm
                        )
                    )
                    valid_flag = False
        ...
        if init_prob_dist is None:
            valid_flag = False
        elif not np.isclose(np.sum(init_prob_dist), 1.0):
    ...
    def train_markov_model(sequence_lst, model_output_filename=None, model_label=""):
        ...
        if model_output_filename is not None:
            if "markov_freq_matrix" in mk_freq_mat:
                mk_freq_mat["markov_freq_matrix"] = list(
                    mk_freq_mat["markov_freq_matrix"]
                )

                for i in range(len(mk_freq_mat["markov_freq_matrix"])):
                    mk_freq_mat["markov_freq_matrix"][i] = [
                        float(a) for a in mk_freq_mat["markov_freq_matrix"][i]
                    ]

                mk_freq_mat["model_label"] = model_label
                mk_freq_mat["model_created_at_utc"] = (
                    datetime.utcnow().isoformat().split(".")[0] + "Z"
                )
                gzipTextFile(
                    model_output_filename, json.dumps(mk_freq_mat, ensure_ascii=False)
                )
    ...
    def mle_get_markov_params(markov_freq_mat, unknown_seq):
        if (
            "vocab" not in markov_freq_mat
            or "markov_freq_matrix" not in markov_freq_mat
            or unknown_seq == ""
        ):
            logger.info(
                "\tmle_get_markov_params(): 'vocab' not in markov_freq_mat or 'markov_freq_matrix' not in markov_freq_mat or unknown_seq == ''"
            )
        ...
        if isinstance(markov_freq_mat["markov_freq_matrix"], list):
            markov_freq_mat["markov_freq_matrix"] = np.array(
                markov_freq_mat["markov_freq_matrix"]
            )

        start_prob_dist = [
            markov_freq_mat["start_dist"][s] for s in markov_freq_mat["vocab"]
        ]
        ...
        markov_freq_mat["markov_freq_matrix"] = np.concatenate(
            (markov_freq_mat["markov_freq_matrix"], z), axis=1
        )

        # add extra rows of zeros to new matrix
        z = np.zeros((new_states_count, len(markov_freq_mat["vocab"])))
        markov_freq_mat["markov_freq_matrix"] = np.concatenate(
            (markov_freq_mat["markov_freq_matrix"], z), axis=0
        )
        ...
        markov_freq_matrix_row_sums = [
            sum(v) for v in markov_freq_mat["markov_freq_matrix"]
        ]
        # new_states
        ...
        if "un_smoothed_markov_freq_matrix" not in markov_freq_mat:
            markov_freq_mat["un_smoothed_markov_freq_matrix"] = deepcopy(
                markov_freq_mat["markov_freq_matrix"]
            )
        ...
        for i in range(n):
            ...
            if 0 in markov_freq_mat["markov_freq_matrix"][i]:
                markov_freq_mat["markov_freq_matrix"][i] = (
                    markov_freq_mat["markov_freq_matrix"][i] + 1
                )
                markov_freq_matrix_row_sums[i] = sum(
                    markov_freq_mat["markov_freq_matrix"][i]
                )
    ...
    @staticmethod
    def get_ngram_freq(
        sequence_lst, ngram_range=(1, 2), token_pattern=r"[^ |()*]", **kwargs
    ):
        kwargs.setdefault("lowercase", False)
        payload = {
            "vocab": [],
            "start_dist": {},
            "ngram_freq": {"all_docs": {}, "all_docs_total_tf": {}},
        }
        ...
        try:
            ...
            payload["tf_matrix"] = count_vectorizer.fit_transform(
                sequence_lst
            ).toarray()
        ...
        for i in range(len(payload["tf_matrix"])):
            ...
            single_doc_tf = [
                {
                    "term": v,
                    "term_freq": int(payload["tf_matrix"][i][j]),
                    "term_rate": payload["tf_matrix"][i][j] / total_tf,
                }
                for (j, v) in enumerate(payload["vocab"])
                if payload["tf_matrix"][i][j] != 0
            ]

            for tf_dct in single_doc_tf:
                payload["ngram_freq"]["all_docs"].setdefault(tf_dct["term"], 0)
                payload["ngram_freq"]["all_docs"][tf_dct["term"]] += int(
                    tf_dct["term_freq"]
                )
                ...
                payload["ngram_freq"]["all_docs_total_tf"][term_ngram] += int(
                    tf_dct["term_freq"]
                )
    ...
    def frm_prob_of_seq_mle_get_markov_params(self, sequence):
        ...
        if self.training_model is None:
            ...
            markov_params = BLOCMarkovChain.mle_get_markov_params(
                self.training_model, sequence
            )
    ...
    def prob_of_sequence(self, sequence, log_prob=True):
        ...
        if (
            self.states is None
            or self.init_prob_dist is None
        ...
        for i in range(1, len(sequence)):
            ...
            na = (
                1
                if self.training_model["un_smoothed_markov_freq_matrix"][
                    cur_indx, next_indx
                ]
                == 0
                else 0
    ...
    def s_prob_of_sequence(training_model, sequence, log_prob=True):
        # Attempt to instantiate states or init_prob_dist or transition_matrix from training_model
        markov_params = BLOCMarkovChain.mle_get_markov_params(
            deepcopy(training_model), sequence
        )
        ...
        un_smoothed_markov_freq_matrix = markov_params.get(
            "un_smoothed_markov_freq_matrix", None
        )
