...
def run_subcommands(args, subcommand, payload):
    bloc_variant = (
        ...
        if args.ngram > 1
            ...
            "type": "folded_words",
            "fold_start_count": args.fold_start_count,
    ...
    bloc_doc_lst = get_bloc_doc_lst(
        bloc_collection,
        bloc_model["bloc_alphabets"],
        src=args.account_src,
        src_class=args.account_class,
    )
...
def feature_importance(vocab, fst_vect, sec_vect, k=10):
    ...
    feat_importance = sorted(feat_importance, key=lambda x: x["score"], reverse=True)[
        :k
    ]
...
def pairwise_usr_cmp(tf_mat):
    ...
    for i in range(len(report)):
        r = report[i]
        logger.info(
            "\t{} vs. {}, (score, feature):".format(
                r["user_pair"][0], r["user_pair"][1]
            )
        )
        ...
        r["feature_importance"] = feature_importance(
            tf_mat["vocab"],
            tf_mat["tf_idf_matrix"][fst_u_indx]["tf_vector"],
            tf_mat["tf_idf_matrix"][sec_u_indx]["tf_vector"],
        )
    ...
    for r in report:
        logger.info(
            "\t{:.4f}: {} vs. {}".format(r["sim"], r["user_pair"][0], r["user_pair"][1])
        )
        fst_u_indx, sec_u_indx = r["user_pair_indx"]
    logger.info("\t------")
...
def print_top_ngrams(tf_mat, k=10):
    def print_top_k_ngrams(top_ngrams, rate_key, skip_pause_glyph=True):
        ...
        for i in range(len(top_ngrams)):
            ...
            line = "\t{:<4} {:.4f} {} ({})".format(
                f"{counter}.", n[rate_key], n["term"], get_word_type(n["term"])
            )
            counter += 1
            logger.info(line)
