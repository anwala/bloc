...
def get_v1_user_obj(v2_users):
    ...
    for u in v2_users:
        ...
        u["created_at"] = datetime.strptime(
            u["created_at"], "%Y-%m-%dT%H:%M:%S.000Z"
...
def get_v1_lite_twt(v2_tweet, all_v1_user_objs, all_referenced_v2_tweets):
    ...
    in_reply_to_screen_name = (
        ...
        else all_v1_user_objs.get(in_reply_to_user_id, {}).get("screen_name", None)
    )
    place = v2_tweet.get("geo", {}).get(
        "place_id", None
    )  # assumption is that geo, coordinates and places have collapsed into place
    payload = {
        "id": v2_tweet["id"],
        ...
        "created_at": datetime.strptime(
            v2_tweet["created_at"], "%Y-%m-%dT%H:%M:%S.000Z"
        ).strftime("%a %b %d %H:%M:%S +0000 %Y"),
    ...
    if len(r_twts) == 1:
        twtype = r_twts[0]["type"]
        rtid = r_twts[0]["id"]
        if twtype == "retweeted" and rtid in all_referenced_v2_tweets:
            payload["retweeted_status"] = get_v1_lite_twt(
                all_referenced_v2_tweets[rtid],
                all_v1_user_objs,
                all_referenced_v2_tweets,
            )
...
def conv_v2_tweets_to_v1(v2_tweets):
    ...
    all_referenced_v2_tweets = get_referenced_v2_tweets(
        v2_tweets.get("includes", {}).get("tweets", [])
    )

    return [
        get_v1_lite_twt(t, all_v1_user_objs, all_referenced_v2_tweets)
        for t in v2_tweets["data"]
    ]
